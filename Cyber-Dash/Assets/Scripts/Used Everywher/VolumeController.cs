...
public class VolumeController : MonoBehaviour
{
    ...
    {
        AD.Master = (float)Math.Round(f, 1) == 0 ? 0.0001f : (float)Math.Round(f, 1);
        Mixer.SetFloat("Master", Mathf.Log10(f) * 20);
    }
    void SFXVolume(float f)
    {
        AD.SFX = (float)Math.Round(f, 1) == 0 ? 0.0001f : (float)Math.Round(f, 1);
        Mixer.SetFloat("SFX", Mathf.Log10(f) * 20);
    }
    void VocalVolume(float f)
    {
        AD.Vocal = (float)Math.Round(f, 1) == 0 ? 0.0001f : (float)Math.Round(f, 1);
        Mixer.SetFloat("Vocal", Mathf.Log10(f) * 20);
    }
    void MusicVolume(float f)
    {
        AD.Music = (float)Math.Round(f, 1) == 0 ? 0.0001f : (float)Math.Round(f, 1);
    ...
    public void MasterMute(bool m)
    {
        Mixer.SetFloat("Master", Mathf.Log10(m ? 0.0001f : AD.SFX) * 20);
    }
}
